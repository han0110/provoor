...
// proof is the VadcopFinalProof ere's verifier accepts, decoded from
// bincode's legacy configuration.
type proof struct {
	proofWords   []uint64
	publicValues []uint64
	compressed   bool
...
// with, whose public values open with the program vk and continue with the
...
func TestTranscodeProofRejects(t *testing.T) {
	cases := []struct {
		...
		{"long vk", func(e *envelope) { e.vkWords = append(e.vkWords, 9) }, "program vk"},
	}
...
// TestTranscodeProofRejectsTruncation sweeps every truncation point, since a
// cut right after a multi-byte varint marker is the case that reads past the end.
func TestTranscodeProofRejectsTruncation(t *testing.T) {
	valid := validEnvelope().encode()
	// The trailing hash_mode byte is never decoded.
...
// TestTranscodeProofRejectsOversizedLength pins that a declared element
// count beyond the remaining input fails at once instead of being looped
// over.
func TestTranscodeProofRejectsOversizedLength(t *testing.T) {
	encoded := []byte{0, 253, 0, 0, 0, 0, 0, 1, 0, 0}
	done := make(chan error, 1)
	go func() {
		_, err := transcodeProof(encoded)
		done <- err
	}()
	select {
	case err := <-done:
