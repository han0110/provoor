...
func TestCoordinatorSpec(t *testing.T) {
	...
	if got := string(spec.Files["/tmp/zisk-coordinator.toml"]); got != coordinatorTOML {
		t.Errorf("Files = %q", got)
	}
	if got := string(spec.Files["/tmp/zisk-coordinator-engine.toml"]); got != coordinatorEngineTOML(testConfig().Workers) {
		t.Errorf("Files = %q", got)
	}
}
...
// recording pkill on PATH, so only the worker's own unhealthy verdict or a
// run of failures after it answered kills the ranks.
func TestWorkerHealthProbe(t *testing.T) {
	stat, err := os.ReadFile("/proc/1/stat")
	if err != nil {
		t.Fatal(err)
	}
	state := "/tmp/zisk-health-" + strings.Fields(string(stat))[21]
	os.Remove(state)
	t.Cleanup(func() { os.Remove(state) })
	...
	run()
	server.Close()
	for range workerHealthMaxMisses - 1 {
		if healthy, killed := run(); healthy || killed {
			t.Fatalf("seen, not listening: healthy = %v, killed = %v, want false and false", healthy, killed)
		}
	}
	if healthy, killed := run(); healthy || !killed {
		t.Errorf("seen, not listening %d times: healthy = %v, killed = %v, want false and true",
			workerHealthMaxMisses, healthy, killed)
	}

	os.Remove(state)
	if healthy, killed := run(); healthy || killed {
		t.Errorf("never seen, not listening: healthy = %v, killed = %v, want false and false", healthy, killed)
	}
}
...
[coordinator]
config_file = "/tmp/zisk-coordinator-engine.toml"
`
	if coordinatorTOML != want {
		t.Errorf("toml =\n%s\nwant\n%s", coordinatorTOML, want)
	}
}

func TestCoordinatorEngineTOML(t *testing.T) {
...
`
	if got := coordinatorEngineTOML(testConfig().Workers); got != want {
		t.Errorf("toml =\n%s\nwant\n%s", got, want)
	}
}
...
func TestSetupSpec(t *testing.T) {
	...
	wantEnv := []string{
		"KEY_URL=https://storage.googleapis.com/zisk-setup/zisk-provingkey-1.2.0-alpha-blake3.tar.gz",
