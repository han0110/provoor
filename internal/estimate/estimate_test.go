...
func TestPendingTests(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "a failure is retried",
			result: &artifact{
				Tests:    map[string]*ereserver.CostEstimation{"first": {}, "third": {}},
...
func TestEstimateAll(t *testing.T) {
	...
	// The stale failure of a test that now succeeds is dropped.
	result := &artifact{
		Tests:    map[string]*ereserver.CostEstimation{},
		Failures: map[string]string{okTest: "guest panicked"},
	}
