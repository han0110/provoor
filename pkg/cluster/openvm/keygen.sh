...
mv /tmp/keygen/program.vmexe "${ARTIFACTS_DIR}/programs/${PROGRAM_NAME}/${PROGRAM_VERSION}/program.vmexe"
for pk in app_pk agg_stark_pk; do
    [ -e "${ARTIFACTS_DIR}/${pk}" ] || mv "/tmp/keygen/${pk}" "${ARTIFACTS_DIR}/${pk}"
done
mv /tmp/keygen/baseline.bin "${ARTIFACTS_DIR}/programs/${PROGRAM_NAME}/${PROGRAM_VERSION}/baseline.bin"
